...
Plug 'dbeniamine/todo.txt-vim'                                                  " \do - Opens ~/todo/todo.txt
Plug 'mbbill/undotree'                                                          " :UndotreeToggle/F5 - Visualise the undo tree
Plug 'vim-pandoc/vim-pandoc-syntax'                                             " Markdown syntax highlighting
...
let g:python3_host_prog = '/usr/bin/python3'                                    " Python providers
...
if not configs.fsharp then
  configs.fsharp = {
    default_config = {
      cmd = {"/home/phil/code/fsharp-language-server/src/FSharpLanguageServer/bin/Release/netcoreapp3.1/FSharpLanguageServer"};
      filetypes = {"fsharp"};
...
nnoremap <Leader>io $XDG_CONFIG_HOME/nvim/plugged/vim-polyglot/indent/fsharp.vim
nnoremap <Leader>ir :runtime indent/fsharp.vim<CR>|                              " Reload FSharp indent file
...
function Run(command)                                                           " Run a build command e.g. 'run', 'test', 'build'
  ...
  execute ':botright split | term ./'.a:command
endfunction
